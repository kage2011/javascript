(() => {
    ...
    kintone.events.on('app.record.detail.show', (event) => {
        ...
        // kintoneの標準ボタンエリアに挿入
        const statusbarAction = document.querySelector('.gaia-app-statusbar-action');
        if (statusbarAction) {
    ...
    button.addEventListener('mouseenter', () => {
        button.style.backgroundColor = '#e3e7e8';
    });
  ...
  async function handleForward(record, appId) {
    try {
        ...
        // 元のレコードの値をコピー（システムフィールドは除外）
        const attachFile = record[CONFIG.ATTACH_FILE_FIELD].value;
