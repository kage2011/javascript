(() => {
    ...
    kintone.events.on('app.record.edit.show', (event) => {
        ...
        cancelBtn.addEventListener('click', async (e) => {
            ...
            if ( copiedTo === recordID){
                ...
                // 元の詳細画面に戻る
                location.href = `/k/${fromappID}/show#record=${copiedFrom}`;
    ...
    async function handleForward(record, appId, fromId) {
        try {
            ...
            // window.open(`/k/402/show#record=${recordID}&mode=edit`, '_blank');
            location.href = `/k/${MailappID}/show#record=${recordID}&mode=edit`;
    ...
    async function handleReply(record, appId, fromId) {
        try{            
           ...
           // 元のレコードの値をコピー
            ...
            // 作成画面を開く
            // window.open(`/k/${MailappID}/show#record=${recordID}&mode=edit`, '_blank');
